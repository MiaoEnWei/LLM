...
# 最终版 (V5):
# - 适配 LLaMA-2 / GPT-2 (8-bit / device_map)
# - 添加 --seed 42 (可复现性)
# - 添加 5 个오답/正确示例打印 (误差分析)
# - 新增 --adapter 参数，可评测 PEFT 模型 (LoRA / Prompt Tuning)
# - 新增 --limit 参数，只评测前 N 条샘플（方便快速调试）
...
def build_args():
    ...
    ap.add_argument(
        ...
        type=int,
        default=42,
        help="设置一个固定的随机种子 (用于可复现性)",
    )
    ap.add_argument(
        "--limit",
        type=int,
        default=0,
        help="如果 >0，只评测前 N 条验证샘플（用于快速调试）",
    )
...
def compute_confusion_and_metrics(
    preds: List[str], gold: List[str], prompts: List[str]
) -> None:
    labels = list(LETTERS)
    L = len(labels)
    ...
    weighted_f = (
        sum(by_cls[c]["f1"] * by_cls[c]["support"] for c in labels) / total
        if total
        else 0.0
    )
    ...
    for c in labels:
        d = by_cls[c]
        print(
            f"  {c}    |  {d['precision']:.4f}  {d['recall']:.4f}  {d['f1']:.4f}    {d['support']:5d}     {d['correct']}/{d['support']}"
        )
    print("\n=== Averages ===")
...
def main():
    ...
    assert len(prompts) > 0, f"解析到 0 条샘플：{args.val}"
    ...
    if args.adapter:
        if not PEFT_AVAILABLE:
            raise ImportError(
                "检测到 --adapter，但未安装 'peft' 库，请先: pip install peft"
            )
    ...
    if args.calib_n and args.calib_n > 0:
        ...
        prior = estimate_prior(
            model, tok, prompts[:n], args.max_len, CAND, args.batch, dev
        )
