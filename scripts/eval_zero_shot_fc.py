...
def build_args():
    ...
    ap.add_argument(
        "--load_in_4bit",
        action="store_true",
        help="bitsandbytes 4-bit quantization (bitsandbytes required)",
    )
    # ------------------------------------
...
def compute_confusion_and_metrics(preds: List[str], gold: List[str], prompts: List[str]) -> None:
    labels = list(LETTERS)
    L = len(labels)
...
def main():
    ...
    for i in range(0, len(prompts), args.batch):
        ...
        for ch in LETTERS:
            ...
            if not idxs:
                ...
                sc = torch.logsumexp(torch.stack([lp[:, j] for j in idxs], dim=1), dim=1)
    ...
    if args.save_jsonl:
        ...
        with open(out_path, "w", encoding="utf-8") as f:
            for i, (pmt, g, pr) in enumerate(zip(prompts, gold, preds)):
