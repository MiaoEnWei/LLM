...
def build_args():
    ...
    ap.add_argument("--batch", type=int, default=24)  # LLaMA-2: 16~32 比较稳
    ap.add_argument("--dtype", default="float16", choices=["auto","float16","bfloat16","float32"])
    ap.add_argument("--calib_n", type=int, default=1500, help="先验样本数；0=关闭校准")
    # —— 关键：新增这两个参数 ——
    ap.add_argument("--device_map", default="auto", choices=["auto","cuda","cpu"],
                    help="auto=分层CPU/GPU（推荐）")
    ap.add_argument("--load_in_8bit", action="store_true", help="bitsandbytes 8-bit 量化以省显存")
...
def parse_prompt_answer(o:Dict)->Optional[Tuple[str, Optional[str]]]:
    ...
    for k in ["answer","label","gold","target","solution","correct"]:
        if k in o: gold=cop_to_letter(o.get(k)); break
...
def parse_medmcqa(o:Dict)->Optional[Tuple[str, Optional[str]]]:
    ...
    body=("You are a medical exam solver. Choose the single best option and reply with only one letter.\n"
          f"Question: {q}\nA) {a}\nB) {b}\nC) {c}\nD) {d}\n")
...
def compute_confusion_and_metrics(preds:List[str], gold:List[str])->None:
    labels=list(LETTERS); L=len(labels); idx={c:i for i,c in enumerate(labels)}
    conf=[[0]*L for _ in range(L)]
    for p,g in zip(preds,gold): conf[idx[g]][idx[p]]+=1
    pred_cnt={c:0 for c in labels}; gold_cnt={c:0 for c in labels}
    ...
    by_cls={}
    for i,c in enumerate(labels):
        TP=conf[i][i]; FP=pred_cnt[c]-TP; FN=gold_cnt[c]-TP
        prec=safe(TP, TP+FP); rec=safe(TP, TP+FN)
    ...
    total=len(gold); acc=sum(conf[i][i] for i in range(L))/total if total else 0.0
    ...
    for i in range(0, len(prompts), args.batch):
        ...
        for ch in LETTERS:
            ...
            if not idx:
                ...
                if prior is not None: sc = sc - prior[ch]
                cols.append(sc)
